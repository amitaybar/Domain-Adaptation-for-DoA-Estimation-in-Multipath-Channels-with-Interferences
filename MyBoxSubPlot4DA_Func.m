function MyBoxSubPlot4DA_Func(Data, Labels, Title,KSubPlots,Groups,LogScaleFlag)

warning('off','MATLAB:Axes:NegativeLimitsInLogAxis'); %due to log scale
set(0,'DefaultTextFontSize',18,'DefaultAxesFontSize',18,'DefaultTextFontName','Times',...
...
if KSubPlots==3
    ...
    boxplot((Data(:,1:SPSize)),'labels',LabelsFunc(Labels,1,SPSize), 'symbol', '','Colors',ColorsArr,'Whisker',0);   
    ax1 = gca;
    set(findobj(gca,'type','line'),'linew',2)
    ylabel('[deg]');
%     title(Title);
%     set(gca, 'FontSize', hcfontsize/1.5);
    set(findobj(gca,'Type','text'),'FontSize',18)%DELETE
    ...
    boxplot(Data(:,SPSize+1:2*SPSize),'labels',LabelsFunc(Labels,SPSize+1,2*SPSize), 'symbol', '','Colors',ColorsArr,'Whisker',0);
...
%     set(gca, 'YLim', get(ax1,'YLim'));
    ...
    boxplot((Data(:,2*SPSize+1:3*SPSize)),'labels',LabelsFunc(Labels,2*SPSize+1,3*SPSize), 'symbol', '','Colors',ColorsArr,'Whisker',0);
    ax3 = gca;
...
%     YLimDown    = max(0,YLimDown); %DELETE
...
%     boxplot(10*log10(Data(:,1:SPSize)),'labels',LabelsFunc(Labels,1,SPSize), 'symbol', '');
    boxplot((Data(:,1: Groups(1))),'labels',LabelsFunc(Labels,1, Groups(1)), 'symbol', '','Colors',ColorsArr);
    for ii=1:Groups(1) 
        plot(ii,mean(Data(:,ii: ii),1),'x');     
    end
    ax1 = gca;
    set(findobj(gca,'type','line'),'linew',2)
    ylabel('Error [deg]');
...
%     set(gca, 'FontSize', hcfontsize/1.5);
    ...
    boxplot((Data(:, Groups(1)+1:sum(Groups))),'labels',LabelsFunc(Labels, Groups(1)+1,sum(Groups)), 'symbol', '','Colors',ColorsArr);
    for ii=1:Groups(1) 
        plot(ii,mean(Data(:,Groups(1)+ii: Groups(1)+ii),1),'x');     
    end
    % boxplot(10*log10(DirectivitySSAll(:,4:6)),'labels',{LabelsML1(4:6)}, 'symbol', '');
    ax2 = gca;
    set(findobj(gca,'type','line'),'linew',2)
...
elseif KSubPlots==1
    ...
    figure;hold on    
    boxplot((Data),'labels',Labels, 'symbol', '','Colors',[0, 0.4470, 0.7410]);
    ax1 = gca;
    set(findobj(gca,'type','line'),'linew',2)
    ylabel('Error [deg]');
%     title(Title);
